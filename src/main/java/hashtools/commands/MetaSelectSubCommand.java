// src/main/java/hashtools/commands/MetaSelectSubCommand.java
...
public class MetaSelectSubCommand implements Runnable {
    ...
    @Option(names = "--copy", paramLabel = "DEST_DIR",
            description = "Copy each selected file into this directory (stripping first 3 path segments)")
    private File copyDir;

    @Override
    public void run() {
        // Validate copyDir if provided
        if (copyDir != null) {
            if (!copyDir.exists() || !copyDir.isDirectory()
                    || !copyDir.canRead() || !copyDir.canWrite()) {
                System.err.printf("ERROR: --copy target must be an existing, readable/writable directory: %s%n",
                        copyDir);
                System.exit(1);
            }
        }
